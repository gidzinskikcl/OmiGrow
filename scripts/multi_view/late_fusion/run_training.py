...
from training.late_fusion_training import train
...
def main():
    parser = argparse.ArgumentParser(description="Run late-fusion dual-view training.")
    parser.add_argument(
        "--model_id",
        type=str,
        required=True,
    ...
    # --------------------------------------------------------
    # Load pretrained encoder params
    # --------------------------------------------------------
    ...
    for seed in SEEDS:
        ...
        X1_model, X2_model, X1_history, X2_history, metrics = train(
            X1=X1_scaled,
            X2=X2_scaled,
            y=y,
    ...
    X1_history_df = pd.DataFrame(final_X1_history.history)
    history_X1_csv_path = os.path.join(
        output_dir, f"{best_seed}_final_X1_training_history.csv"
    )
    X2_history_df = pd.DataFrame(final_X2_history.history)
    history_X2_csv_path = os.path.join(
        output_dir, f"{best_seed}_final_X2_training_history.csv"
    )
    X1_history_df.to_csv(history_X1_csv_path, index=False)
    X2_history_df.to_csv(history_X2_csv_path, index=False)
    print(
        f"Saved final training history to: {history_X1_csv_path} and {history_X2_csv_path}"
    )

    # Save final weights
    os.makedirs("weights", exist_ok=True)
    weights_X1_path = os.path.join(
        "weights",
        f"{model_id}_{modality_1}_{best_seed}.weights.h5",
    )
    weights_X2_path = os.path.join(
        "weights",
        f"{model_id}_{modality_2}_{best_seed}.weights.h5",
    )
    final_X1_model.save_weights(weights_X1_path)
    print("Saved model weights to:", weights_X1_path)
    final_X2_model.save_weights(weights_X2_path)
    print("Saved model weights to:", weights_X2_path)
